...
var FileRemover = require('../utils/rotatingFileRemover').FileRemover;
...
function RotatingFileHandler(options) {
  ...
  if(this._timeRate && options.maxHistory) {
    this._maxHistory = options.maxHistory;
    this._remover = new FileRemover({ fileFormat: this._oldFile, defaultDateFormat: '%Y%m%d%H%M%S%L' });
  }
}
...
RotatingFileHandler.prototype._deleteOldFiles = function(date) {
  return this._remover ? this._remover.deleteOldFiles(date) : Promise.fulfilled();
};
...
RotatingFileHandler.prototype._getData = function() {
  if(this._size === undefined || this._time === undefined) {
    var that = this;
...
RotatingFileHandler.prototype.rotate = function _rotate() {
  ...
  var oldestFilesMtime = new Date();
  if(this._maxHistory) {
    var nextRotTime = that._nextRotate(that._rotateAt).getTime();
    var rotDiff = nextRotTime - this._rotateAt.getTime();
    oldestFilesMtime = new Date(this._rotateAt.getTime() + rotDiff * this._maxHistory);
  }

  return that._deleteOldFiles(oldestFilesMtime).then(function() {
...
RotatingFileHandler.prototype._rename = function _rename(num) {
  ...
  if (this._maxIndex && num + 1 >= this._maxIndex) {
    return unlink(name);
  }
  var that = this;
